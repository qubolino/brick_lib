...
module block(
    ...
    roadway_x=0,
    roadway_y=0,
    stud_rescale=STUD_SCALE_UP,
    spline_rescale=SPLINE_SCALE_DOWN,
    dual_sided=false,
    dual_bottom=false,
    ...
    spline_length = (brand == "lego" ? 0.25 : 1.7) * spline_rescale;
    spline_thickness = (brand == "lego" ? 0.7 : 1.3) * spline_rescale;
    ...
    real_width = (type == "wing" || force_orientation) ? width : min(width, length);
    real_length = (type == "wing" || force_orientation) ? length : max(width, length);
        ...
        union() {
            difference() {
                union() {
                    ...
                    else if (shape == "cylinder" && width == length) {
                        translate([overall_length/2, overall_width/2, 0])
                        difference() {
                            cylinder(d=overall_length, h=real_height * block_height);
                            cylinder(d=overall_length-wall_thickness*2, h=block_height*real_height-roof_thickness);
                        }
                    }
                ...
                else if (type == "invslope") {
                    translate([0, overall_width+0.5, 0]) rotate([90, 0, 0]) linear_extrude(overall_width+1) polygon(points=[
                        [-0.1, 0],
                        [min(overall_length, overall_length - (stud_spacing * real_slope_stud_rows) + (wall_play/2)), 0],
                        // [min(overall_length, overall_length - (stud_spacing * real_slope_stud_rows) + (wall_play/2)), real_height * block_height + stud_height + 1],
                        [-0.1, real_height * block_height-1]
                    ]);
                }
                else if (type == "curve") {
            ...
            if (type == "wing") {
                ...
                translate([0, overall_width, 0]) rotate([90, 0, 0]) linear_extrude(overall_width) polygon(points=[
                    [0, real_height * block_height],
                    [0, (real_height * block_height) - roof_thickness],
                    [min(overall_length, overall_length - (stud_spacing * real_slope_stud_rows) + (wall_play/2)), 0],
                    [min(overall_length, overall_length - (stud_spacing * real_slope_stud_rows) + (wall_play/2)), roof_thickness]
                ]);
            }
            else if (type == "curve") {
                if (real_curve_type == "concave") {
